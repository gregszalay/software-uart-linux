 ...
 * @param gpio_txde GPIO pin used as TXDE
 * @return 1 if the initialization is successful. 0 otherwise.
 */
int raspberry_soft_uart_init(const int _gpio_tx, const int _gpio_rx, const int _gpio_txde)
{
  bool success = true;
  ...
  if (gpio_txde)
  {
    success &= gpio_request(gpio_txde, "soft_uart_txde") == 0;
    success &= gpio_direction_output(gpio_txde, 0) == 0;
  }
